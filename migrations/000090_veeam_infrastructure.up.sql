...
COMMENT ON COLUMN veeam_infrastructure.host_name IS 'The Proxmox node a worker was deployed to ("hv01.example.lan"), or the literal "This server" for a proxy role the VBR server fills itself. Informational: Veeam''s node naming does not reliably match Nexara''s, so it is not used to resolve the guest';
