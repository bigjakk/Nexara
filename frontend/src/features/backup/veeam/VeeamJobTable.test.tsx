...
describe("VeeamJobTable", () => {
  it("shows job state and result without expanding", () => {
    renderWithProviders(<VeeamJobTable jobs={[job()]} />);
  ...
  it("never labels a failure without acknowledging it may be a cancellation", () => {
    renderWithProviders(
      <VeeamJobTable jobs={[job({ last_result: "Failed" })]} />,
    );
  ...
  it("shows a progress bar only while a job is running", () => {
    ...
    rerender(<VeeamJobTable jobs={[job({ status: "Stopped" })]} />);
    expect(screen.queryByText("Running")).not.toBeInTheDocument();
  });

  it("says so when a server has no Proxmox jobs", () => {
    renderWithProviders(<VeeamJobTable jobs={[]} />);
...
describe("VeeamSessionTable", () => {
  it("distinguishes a Nexara-initiated stop from an unexplained failure", () => {
    const { rerender } = renderWithProviders(
      <VeeamSessionTable
        sessions={[session({ result: "Failed", nexara_initiated: true })]}
      />,
    );
    ...
    rerender(
      <VeeamSessionTable
        sessions={[session({ result: "Failed", nexara_initiated: false })]}
      />,
    );
  ...
  it("explains the ambiguity in the expanded row, but only when it applies", async () => {
    const user = userEvent.setup();
    const { rerender } = renderWithProviders(
      <VeeamSessionTable sessions={[session({ result: "Failed" })]} />,
    );
    await user.click(screen.getByText("Onsite_Daily"));
    expect(
    ...
    rerender(<VeeamSessionTable sessions={[session({ result: "Success" })]} />);
    expect(
  ...
  it("shows the backup mode and transferred size", () => {
    renderWithProviders(<VeeamSessionTable sessions={[session()]} />);
